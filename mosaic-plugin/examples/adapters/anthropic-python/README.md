# mosaic-anthropic-adapter

Phase 4B Python reference adapter — consumes the Mosaic plugin's content
and authors Mosaic YAML models via the Anthropic Python SDK.

This is the **default provider** per [ADR-0008](../../../../docs/decisions/0008-phase-4-llm-authoring-and-plugin-ecosystem.md) amendment D. The OpenAI adapter at
[`../openai-python/`](../openai-python/) is the cross-provider proof.
...
The only first-party dependency is the `anthropic` SDK (per ADR-0008
amendment G — no `pyyaml` / `pydantic` / `httpx` / etc.).

## Set the API key

```bash
export ANTHROPIC_API_KEY=...   # NEVER pass via CLI arg; never log
```
...
2. Appends a response-format instruction telling the LLM to emit YAML in a
   single `` ```yaml ``...`` ` `` `` fenced block with no surrounding prose.
3. Calls Anthropic's `messages.create` with the system prompt + the user's
   prompt.
...
[mosaic] calling claude-opus-4-7 (initial draft)...
...
- **`anthropic.AuthenticationError`** — set `ANTHROPIC_API_KEY` in the env.
- **Convergence failure (max_iterations exceeded)** — the last failed YAML
  ...
  string is stale (Anthropic ships new flagships ~quarterly — verify
  `MODEL` in `author.py` is current), API key has insufficient credit.
