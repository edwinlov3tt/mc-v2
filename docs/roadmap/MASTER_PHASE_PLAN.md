...
| **10C / 10D / 10E** | Remaining evaluation commands, demand-driven: `backtest` (parameter sweep × holdout, claw-core's #2 ask — α/threshold/Kelly sweeps), `sweep --games` (batch per-game sensitivity), `walk-forward` (Mosaic-side multi-fold record generation — would *emit* the 10F bet-record format). Each independently shippable; sequenced by consumer demand. Per research note `built-in-evaluation-primitives.md`. | not started — pending demand | — |
