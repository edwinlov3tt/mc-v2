...
| **4C** | Organization and workspace primitive — rescoped from "multi-domain workspace" per ADR-0026. `org.yaml` + `workspace.yaml` manifest, capability-grant model, five-level template inheritance, resource scoping, managed-org relationships. `mc workspace {init,validate,lint,test,inspect}` verbs. Foundation for Phase 8 daemon (must be org-aware) and Phase 9 cloud (tenant = org). Per ADR-0026. | planned (ADR-0026 proposed — needs acceptance before implementation) | — |
| **5D** | Tessera XLSX driver + layout descriptors + aggregation transforms — `DriverKind::Xlsx` via calamine crate, `skip_rows`/`header_row`/`sheet` fields on SourceConfig (also benefits CSV), `group_by`/`aggregate` post-fetch transforms. Closes 238-line Python workaround. Per ADR-0028. | planned (ADR-0028 proposed) | — |
...
| **8** | Service daemon — long-running process with hot cube cache, LRU eviction, snapshot persistence, crash recovery. API/MCP access, org-aware (inherits Phase 4C). Requires Phase 4C to be complete. Per ADR-0025 Shape 4. | not started (ADR needed; requires Phase 4C + Phase 7A completion) | — |
