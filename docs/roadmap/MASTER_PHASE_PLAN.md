...
| **3C** | Model Test Fixtures and Input Sets — `canonical_inputs` + `test_fixtures`, sibling CSV + tabular inline YAML, 14 new validators (MC2012–MC2025), `mc model test --fixture <name>` (per ADR-0006) | **complete** (report at [`../reports/phase-3c-completion-report.md`](../reports/phase-3c-completion-report.md)) | (uncommitted; awaiting user review) |
| **3D** | Friendly formula syntax — `Revenue = Customers * AOV` strings compile to `ParsedRuleBody`'s structured tree (originally ADR-0004's Phase 3C; renamed to 3D per ADR-0006 roadmap impact) | proposed | — |
