...
### 3A — Declarative model format + parser (planned, blocked)

- **Status:** planned, **blocked**. Two preconditions must clear before Phase 3A flips to `proposed`: (1) Phase 2D ships *or* is formally deferred (no Phase 3A work begins while §9.3 is open against measured Phase 2C data); (2) the format/parser ADR lands (the choice between TOML+serde, a custom DSL, `pest` / `nom` / hand-rolled, etc., plus the keep-out-of-`mc-core` invariant). The Phase 2 housekeeping Q2 (toolchain bump) is also a Phase 3A precondition: a parser dep that requires `edition2024` will force a Rust 1.85+ bump, and that decision belongs in 3A's ADR, not in an ad-hoc dep add.
