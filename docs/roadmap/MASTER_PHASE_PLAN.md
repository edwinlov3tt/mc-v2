...
**Last updated:** 2026-05-08 (post-Phase 7A.4 benchmark aggregation — **PHASE 7A.1-7A.4 COMPLETE**; 1001 / 0 / 5 tests. Narrative intelligence arc ships: template evaluation → ledger → trends → own-workspace benchmarks. Next: 7A.5 explanation chains, then 7B visual editor.)
...
| **7A.5** | Explanation chains + context events — causal attribution for narrative findings. `finding_id` + `explanation_priority` fields on templates; prioritized evaluation (first match wins). `context-events.yaml` loader + `has_context_event()`/`context_description()` functions. Auto-detection of input-change events (budget change → synthetic event). Per research note at `docs/research-notes/explanation-chains-and-template-versioning.md`. | planned (needs ADR) | — |
| **7B** | Visual template editor + versioning — UI for authoring narrative templates. Live preview against current cube state. Template versioning (`narrative_format_version: 2`, `version`/`deprecated`/`supersedes` fields). LLM-assisted template generation. | planned (after 7A.5 + 6B) | — |
