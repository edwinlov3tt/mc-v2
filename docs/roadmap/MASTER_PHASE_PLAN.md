...
| **3A** | Model definition layer — YAML + `mc-model` crate (per ADR-0004) | **proposed** (handoff at [`../handoffs/phase-3a-handoff.md`](../handoffs/phase-3a-handoff.md)) | — |
...
- **Status:** **proposed.** Both Phase 3A preconditions cleared on 2026-05-02: (1) Phase 2D shipped at `0678a98` (tag `phase-2d-bitset-and-invalidated-fix`); (2) [ADR-0004](../decisions/0004-phase-3a-model-definition-format.md) Accepted, fixing the format (YAML safe subset), parser crate (`mc-model`), dep boundary (no serde in `mc-core`), rule shape (structured expression trees, formula strings deferred to Phase 3C), pipeline (3-stage: YAML → ParsedModel → ValidatedModel → Cube), validator surface, golden-test format (inline default), and out-of-scope cliffs. Handoff at [`../handoffs/phase-3a-handoff.md`](../handoffs/phase-3a-handoff.md). Phase 2 housekeeping Q2 (toolchain bump) is now a *conditional* Phase 3A precondition — only if the chosen YAML library can't build on Rust 1.78 with transitive pins (per ADR-0004 Decision 3); if it triggers, Phase 3A blocks on ADR-0005 before any source change.
