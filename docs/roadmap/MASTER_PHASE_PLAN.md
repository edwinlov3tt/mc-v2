...
**Last updated:** 2026-05-06 (post-Phase 3H.2 fitted-model adstock + saturation — **PHASE 3 COMPLETE**; 912 / 0 / 5 tests; HEAD `d240802`. Formula-engine deferred queue from ADR-0015 is empty. Project transitions to "demand-driven only" for formula work; next-phase candidates: 4C, 5D, 6B, 6C.)
...
| **4D** | Verbose CLI mode — `--verbose` / `-v` flag on existing Phase 6A verbs producing prose summaries from measure `description:` fields; optional `description:` substitution with `{value}` placeholder. Small phase (~2-3 days). Does NOT require `mc-narrative` crate. | planned | — |
...
| **7A.1** | Narrative engine crate (`mc-narrative`) — extract `narrative.rs` from `mc-demo-server` into a standalone library crate. Public API: `evaluate_templates(templates, cube, refs) -> Vec<NarrativeOutput>`. New CLI verb `mc model narrate`. New MCP tool `mosaic.model.narrate`. Template YAML parser + validator (MC7xxx codes). The proper productionization of Phase 6D's demo narrative engine. | planned (after 6D ships) | — |
| **7A.2** | Narrative ledger — persistent structured analysis events (JSONL per-period). Every narrative output logged with evidence objects, template IDs, severity, benchmarks. Enables cross-period queries (`mc model query-ledger --severity warning --repeated 3`). | planned (after 7A.1) | — |
| **7A.3** | Cross-period analysis — trend detection from narrative ledger ("Paid Search has declined for 3 consecutive months"). Deterministic rules firing against prior ledger entries + current cube values. `mc model narrate-trends --last 6-periods`. | planned (after 7A.2) | — |
| **7A.4** | Benchmark aggregation — privacy-aware multi-tenant benchmark library built from anonymized narrative evidence trails. Internal benchmarks emerge from report history. The moat. Needs its own ADR for privacy model. | planned (after 7A.3; needs ADR) | — |
| **7B** | Visual template editor — UI for authoring narrative templates without YAML. Live preview against current cube state. Version control on templates. Template sharing across teams. | planned (after 7A.1 + 6B) | — |
| **4C** | Multi-domain workspace primitive — `workspace.yaml` manifest + `$ref:` resolver for shared catalogs + `mc workspace {init,validate,lint,test,inspect}` verbs + domain pack format. Per proposal at `docs/research-notes/multi-domain-workspaces-proposal.md`. Tier A scope (no kernel changes). | planned (needs ADR) | — |
| **5D** | Tessera driver expansion — XLSX driver, year-blocked layouts, multi-file ingest, aggregation transforms (`group_by`). Closes ~420 lines of email-matchback Python. | planned (needs ADR) | — |
