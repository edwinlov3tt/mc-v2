...
| **10A** | Evaluation metrics library — 5 new formula primitives (`std_over`, `var_over`, `count_over`, `wilson_ci_lower`, `wilson_ci_upper`) + metrics cookbook documenting compositional patterns for direction_accuracy, ROI, Brier, Sharpe, etc. Foundational for the evaluation-primitives track (Phase 10B-F). Demand-driven sequencing (Option 3 from research-note analysis of 29 claw-core MLB experiment scripts). Hand-rolled, no new deps, mirrors Phase 3L precedent. ~150 LOC + tests. Per ADR-0033 (7 amendments). | **complete** | `3662c78` merged + fix `2a92c6d` |
| **10B** | `mc model grade` — segmented holdout evaluation. Grouped map-reduce over a unit dimension: group by dimension/measure/bucket, compute per-segment metrics (count, mean, sum, ratio, std, min, max, wilson_lower/upper — 9 reductions) via the 10A primitives, flag segments crossing a threshold, emit text table + expanded JSON. Reproduces claw-core EXP-048/022/023/031c segment tables in one command. CLI-only (no daemon, no kernel change). Reuses existing `Filter` grammar; `--bucket`+`--max-segments` guard continuous grouping; Wilson Null hard-errors; `LoadPolicy::Reproducible` default. ~2570 LOC. Per ADR-0034 (12 amendments). **Adopted by claw-core** (reproduced EXP-048 live; named simulate as #1 next ask). | **complete** | `951118e`+`d0b8a83` merged `c443d1f` |
| **10F** | `mc model simulate` — chronological bankroll simulation. Path-dependent bet-record replay (grade's time-ordered sibling). Consumes a bet-record file (parquet/jsonl, NOT the cube), sizes per-bet via Kelly vocabulary (flat/kelly/quarter/half + cap/shrink/min_odds/floor), tracks bankroll curve, computes max_drawdown/recovery_bets (deferred from 10A) + sharpe + Monte Carlo percentile bands (iid/block resampling). 4-state outcome enum w/ `won`-0/1 compat. Format grounded in claw-core's real `exp028_bets.parquet`. claw-core's #1 ask after grade — V1.1-gating headline numbers are path-dependent bankroll numbers. CLI-only, ~600-700 LOC. Per ADR-0035 (17 amendments — same-timestamp batch, `--replay batch\|sequential`, bankruptcy/ruin, 4-state-required, pinned splitmix64, DuckDB-in/jsonl-out). **EXP-049 reproduced byte-identically** ($2962.1597 2025-only, $137,053 3-season — both verified against claw-core's real file via the merged binary). 34-ACs, ~3,900 LOC, zero mc-core, zero new deps. | **complete** | `6e2d3e9` merged `7462c22` |
| **10F.1** | Push-accuracy default + EXP-029 gaps (ADR-0035 Amdts 18-19). Responds to claw-core's 38% overstatement catch: push-accuracy is now the DEFAULT when score columns present (auto-derive `(actual-line).abs()<1e-9`; `--no-derive-pushes` opts out), `win_rate` excludes pushes, escalated legacy-binary warning, `--max-stake` (029d) + count-based `--window first:n` (029e). EXP-049 paired: legacy $2,962 (audit) + push-accurate $1,830 sequential / $1,829 batch (correct). mc-cli only, simulate 30→36 tests. **PM independently re-verified both numbers via merged binary.** Validation finding captured in `docs/research-notes/evaluation-oracle-validation-push-bug.md`. | **complete** | `a22beed` merged `4534dd3` |
| **10C** | `mc model backtest` — parameter sweep × holdout evaluation. Sweep a param/coefficient/input axis across a grid; at each grid point run the full grade-style holdout evaluation; report the metric surface + flag the best setting (`--objective`/`--goal`). Composes grade (per-point eval, 9-reduction vocab, Filter grammar) + sweep (axis mechanics); opt-in `--simulate` objective for path-dependent metrics. **Multi-domain by mandate** — engine ships no domain metric; betting/marketing/forecasting/finance use it unchanged (AC #15 non-betting test + AC #22 dual cookbook example are the guardrails). Replaces ~5-6 no-refit scripts (EXP-032/033/039/042/044/045; EXP-021/026 need a deferred `variant:` axis — they sweep training hyperparameters that require refit). CLI-only. Per ADR-0036 (8 amendments, SPIKE-GATED). **Dual codebase-access review (Codex + Claude Code) confirmed the multi-domain spine is clean BUT the primary `param:` axis has no setter and params are outside dirty propagation (cube.rs:3069) — so 10C is split: 10C.0 spike proves param-recompute first; only then does 10C.1 build the command.** `--simulate` deferred (reads external file a cube sweep can't change). +rmse reduction, +values-list axis, +best-by-segment. | 10C.0 spike GREEN (zero kernel change); 10C.1 ready | — |
| **10C.0** | Param-recompute spike — the prototype that gates 10C.1. Answers ONE question: does a `param(name)`-dependent derived measure recompute when the param changes, or serve stale cache? (cube.rs:3069 says params don't participate in dirty propagation.) Outcome GREEN (zero-kernel-change → 10C.1 is composition) / YELLOW (small mc-core additive) / RED (kernel ADR needed → re-scope to coef/input axes for v1). ~0.5-1 session; deliverable is a verdict report, not product code. Per ADR-0036 Amdt 1. **🟢 GREEN — zero kernel change.** rollback_to (cube.rs:2802) clears all caches, so param: sweep recomputes via the snapshot→rollback→set→read pattern coef: already uses. 10C.1 is a CLI composition. Report at `docs/reports/phase-10c0-spike-report.md`. | **complete** | merged to main |
| **10D / 10E** | Remaining evaluation commands, demand-driven: `sweep --games` (batch per-game sensitivity), `walk-forward` (Mosaic-side multi-fold record generation — refits per fold in Python, emits the 10F bet-record format; backtest is the no-refit cousin). Sequenced by consumer demand. Per research note `built-in-evaluation-primitives.md`. | not started — pending demand | — |
