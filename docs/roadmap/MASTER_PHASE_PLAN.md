...
| **10B** | `mc model grade` — segmented holdout evaluation. Grouped map-reduce over a unit dimension: group by dimension/measure/bucket, compute per-segment metrics (count, mean, sum, ratio, std, min, max, wilson_lower/upper — 9 reductions) via the 10A primitives, flag segments crossing a threshold, emit text table + expanded JSON. Reproduces claw-core EXP-048/022/023/031c segment tables in one command. CLI-only (no daemon, no kernel change). Reuses existing `Filter` grammar; `--bucket`+`--max-segments` guard continuous grouping; Wilson Null hard-errors; `LoadPolicy::Reproducible` default. ~400-500 LOC. Per ADR-0034 (12 amendments). | not started — accepted, ready for handoff | — |
