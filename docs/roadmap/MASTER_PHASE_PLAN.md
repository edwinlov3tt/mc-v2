...
| **10B** | `mc model grade` — segmented holdout evaluation. Grouped map-reduce over a unit dimension: group by dimension/measure/bucket, compute per-segment metrics (count, mean, ratio, std, wilson_lower/upper) via the 10A primitives, flag segments crossing a threshold, emit text table + JSON. Reproduces claw-core EXP-048/022/023/031c segment tables in one command. CLI-only (no daemon). ~400-500 LOC. Per ADR-0034. | not started — proposed | — |
