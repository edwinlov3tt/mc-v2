...
| [0036](./0036-phase-10c-model-backtest.md) | Phase 10C — `mc model backtest` (Parameter Sweep × Holdout Evaluation) | **Proposed** — sweep a parameter (param/coefficient/input axis) across a grid, run full grade-style holdout evaluation at each point, report the metric surface + best setting. Composes grade (per-point eval) + sweep (axes). **Multi-domain mandate:** engine ships no domain metric; works for betting / marketing / forecasting / finance unchanged. Replaces 7 claw-core scripts. Review request at [`../reviews/adr-0036-review-request.md`](../reviews/adr-0036-review-request.md) |
