...
| [0034](./0034-phase-10b-model-grade.md) | Phase 10B — `mc model grade` (Segmented Holdout Evaluation) | **Proposed** — second phase of the evaluation track; grouped map-reduce over a unit dimension (group by dimension/measure/bucket → per-segment metrics via 10A primitives). Reproduces claw-core EXP-048 segment tables. CLI-only, ~3-4 sessions |
