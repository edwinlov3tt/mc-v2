...
| [0035](./0035-phase-10f-model-simulate.md) | Phase 10F — `mc model simulate` (Chronological Bankroll Simulation) | **Proposed** — path-dependent bet-record replay (grade's time-ordered sibling). Consumes a bet-record file (not the cube), sizes per-bet via Kelly vocabulary, tracks bankroll curve + max_drawdown/recovery (deferred from 10A), Monte Carlo resampling. Format grounded in claw-core's real `exp028_bets.parquet`. claw-core's #1 ask after adopting grade. Review request at [`../reviews/adr-0035-review-request.md`](../reviews/adr-0035-review-request.md) |
