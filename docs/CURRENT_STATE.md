...
**Last updated:** 2026-05-03 (Phase 4B — Python Reference Adapters — **complete** at `b5b6229` (tag `phase-4b-python-adapters`). Anthropic + OpenAI adapters under `mosaic-plugin/examples/adapters/{anthropic,openai}-python/` consume the Phase 4A plugin content and author working Mosaic YAML via two providers. **Headline best-of-3 acceptance gate: Anthropic 3/3 ✓ (post-fix runs at 2/1/4 iter), OpenAI 3/3 ✓ (1/1/1 iter). Both adapters cleared the gate.** Two Phase 4B adapter bugs caught and fixed in-flight (case-insensitive severity filter mismatch + truncation-tolerant YAML extraction; pre-fix Anthropic artifacts archived as `run-anthropic-N-pre-fix.failed.{log,yaml}`); one plugin-doc inconsistency surfaced as a Phase 4A.1 follow-up candidate (NOT folded into 4B). Rust workspace 0-line diff vs `phase-4a-mosaic-plugin`; plugin's `skills/` / `agents/` / `commands/` / `.claude-plugin/` / `.mcp.json` / `examples/models/` / `hooks/` 0-line diff. `cargo test --workspace` still 416/0. Committed at `b5b6229` (tag `phase-4b-python-adapters`). See [`reports/phase-4b-completion-report.md`](./reports/phase-4b-completion-report.md) + [`reports/phase-4b-proof/`](./reports/phase-4b-proof/). Earlier today: Phase 4A — Mosaic Claude Code plugin — committed at `36af56c` (tag `phase-4a-mosaic-plugin`). Project renamed MarketingCubes V2 → Mosaic; ADR-0008 Accepted with 9 amendments. Earlier this week: Phase 3D `d5ab355`; Phase 3C `8d2691a`; Phase 3B `f4f7fa8`.)
...
**Phase 4A commit / tag:** `36af56c` (tag `phase-4a-mosaic-plugin`)
...
- **Phase 4A — Mosaic Claude Code plugin.** **Complete 2026-05-03, committed at `36af56c` (tag `phase-4a-mosaic-plugin`).** See [`reports/phase-4a-completion-report.md`](./reports/phase-4a-completion-report.md). Ships `mosaic-plugin/` at workspace root: manifest at `.claude-plugin/plugin.json` (canonical Claude Code format observed in cached vercel/0.40.1 + superpowers/5.0.7 references); 6 skills (authoring, debugging, schema-design, formulas, testing, marketing-mix domain); 4 agents (architect/author/debugger/validator); 6 commands (init/validate/inspect/lint/test/author; `/mosaic-explain` deferred to Phase 4A.2 — needs `mc model trace` verb); `.mcp.json` invoking `mc mcp`; Acme example byte-identical to `crates/mc-model/examples/acme.yaml`; hooks/ + examples/adapters/ placeholders for Phase 4A.1 + 4B respectively. **Single Rust addition:** `mc-cli` gains `mc mcp` subcommand at `crates/mc-cli/src/mcp.rs` — 318-line hand-rolled JSON-RPC parser body (over the 250 trigger #10 budget; user-authorized as scope-specific decision per the working-state evidence) + 66-line emitter; surfaces `mosaic.demo / mosaic.model.{validate,inspect,lint,test}` as MCP tools wrapping the existing CLI verbs. Reuses Phase 3B's `diagnostics_to_json` envelope verbatim. **No new Rust deps; no toolchain bump.** Test count **396 → 416** (+20: 8 `tests/mcp_smoke.rs` + 2 `tests/example_byte_identity.rs` + 10 `tests/plugin_lint.rs`). 10/10 deterministic. **Locked surfaces 0-line diff vs `5ea0f02` rename baseline** (the inherited 55-line diff vs `phase-3d-friendly-formula-syntax` is entirely the rename commit's Cargo.toml/lib.rs description updates that landed before Phase 4A started). End-to-end proof: in-session authored `MyCo_Marketing_Q1_2026` (3 channels × 3 markets × Q1 2026) from plugin content alone — validate/lint/test all green; full transcript + YAML at [`reports/phase-4a-proof/`](./reports/phase-4a-proof/). Real fresh-instance verification deferred to user post-review. Marketing-mix is the ONLY domain schema (per ADR-0008 amendment F).
- **Phase 4B — Python Reference Adapters.** **Complete 2026-05-03, committed at `b5b6229` (tag `phase-4b-python-adapters`).** See [`reports/phase-4b-completion-report.md`](./reports/phase-4b-completion-report.md). Ships two reference adapters under `mosaic-plugin/examples/adapters/`: `anthropic-python/` (default per ADR-0008 amendment D; `claude-opus-4-7` via Anthropic SDK) + `openai-python/` (cross-provider proof per amendment G; `gpt-5.5` via OpenAI SDK `responses.create`). Each is a single self-contained `author.py` (267/263 lines post-fix) + a PEP 621 `pyproject.toml` declaring exactly one first-party dep + a README. Both adapters consume the SAME plugin content (138K-char system prompt = skills + agents + commands + Acme example + binding ```yaml-fence response-format instruction), iterate against `mc model {validate,lint,test} --format json` (subprocess) up to 5 rounds. **Headline best-of-3 acceptance gate: Anthropic 3/3 ✓ (post-fix runs at 2 / 1 / 4 iter), OpenAI 3/3 ✓ (1 / 1 / 1 iter).** Anthropic produced `B2C_SaaS_Marketing_FY26` (SaaS-funnel naming: Trial_Rate / Subscribers / ARPU; Q4 lift via test_fixture; single-element Market); OpenAI produced `B2C_SaaS_Marketing_FY2027` (Acme-faithful naming: CVR / Customers / AOV; Q4 lift via Scenario element; 3-city + region Market hierarchy) — both pass `mc model validate / lint / test` with 10/10 goldens, demonstrating portability. **Two real adapter bugs caught and fixed in-flight** by the gate-run discrepancy: (1) case-insensitive severity filter mismatch (`mc-cli` emits PascalCase; adapter filtered for lowercase per plugin-doc shape), (2) brittle YAML extraction on truncated responses (`MAX_TOKENS=8000` insufficient for Opus 4.7; bumped to 16000 + added fallback regex for opening-fence-only). Pre-fix Anthropic artifacts archived as `run-anthropic-N-pre-fix.failed.{log,yaml}`. **One plugin-doc inconsistency surfaced as Phase 4A.1 follow-up candidate** (`skills/debugging/SKILL.md` documents lowercase severity; actual envelope is PascalCase; NOT folded into 4B per SPEC QUESTION trigger #2). **Rust workspace LOCKED** (0-line diff vs `phase-4a-mosaic-plugin`); plugin's `skills/` / `agents/` / `commands/` / `.claude-plugin/` / `.mcp.json` / `examples/models/` / `hooks/` LOCKED (0-line diff). `cargo test --workspace` still 416/0; toolchain still Rust 1.78. No new Rust deps. No async/streaming/concurrency/retries/rate-limiting/telemetry/cost-tracking. Marketing-mix domain only (per amendment F). Per-run logs + canonical YAMLs + verify.sh harness at [`reports/phase-4b-proof/`](./reports/phase-4b-proof/).
