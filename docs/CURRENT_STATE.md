...
**Last updated:** 2026-05-05 — Phase 3H shipped (fitted-model evaluation: `predict()` + `calibrate()` + `exp()` + `norm_cdf()`, tag `phase-3h-fitted-model-evaluation` at `99477ef`). Formula engine now covers conditionals, time-series, reference-data blocks, runtime time anchor, and fitted-model functions across six expansion phases (3E–3H). Phase 5C shipped (cron scheduling + 5 new Tessera drivers + incremental loads + ADR-0014 `time_format` enforcement, tag `phase-5c-driver-expansion`). Phase 3E–3G shipped (formula language expansion bundle: conditionals + time-series + reference-data blocks, tag `phase-3e-3f-3g-formula-expansion`). Phase 3F.1 shipped (runtime time anchor). **632 / 0 tests.** 7 crates in workspace: `mc-core`, `mc-fixtures`, `mc-model`, `mc-cli`, `mc-recipe`, `mc-drivers`, `mc-tessera`. Toolchain stays Rust 1.78.
...
- Phase 3E–3G: `334e0aa` (tag `phase-3e-3f-3g-formula-expansion`) / Phase 3F.1: `8adbe2b` / Phase 3H: `da644c2` (tag `phase-3h-fitted-model-evaluation`)
- Phase 4A: `36af56c` (tag `phase-4a-mosaic-plugin`) / Phase 4B: `b5b6229` (tag `phase-4b-python-adapters`)
- Phase 5A: `6c9950d` (tag `phase-5a-tessera-core`) / Phase 5B: (tag `phase-5b-llm-recipe-authoring`) / Phase 5C: `e6f72ea` (tag `phase-5c-driver-expansion`)

**Branch:** `main` (tracking `origin/main` at github.com/edwinlovettiii/mc-v2)
...
- **Phase 2 housekeeping — Q2 (toolchain bump).** Deferred; Phase 3A solved with Cargo.lock pins instead.
- **Phase 3D — Friendly formula syntax.** Complete 2026-05-03, `d5ab355` (tag `phase-3d-friendly-formula-syntax`). Hand-rolled recursive-descent parser; MC1003–MC1006; Acme rules now formula strings. 62 new tests; 396/0 total. See [`reports/phase-3d-completion-report.md`](./reports/phase-3d-completion-report.md).
- **Phase 4A — Mosaic Claude Code plugin.** Complete 2026-05-03, `36af56c` (tag `phase-4a-mosaic-plugin`). `mosaic-plugin/` with 6 skills, 4 agents, 6 commands, `.mcp.json`; `mc mcp` JSON-RPC subcommand. 416/0 tests. See [`reports/phase-4a-completion-report.md`](./reports/phase-4a-completion-report.md).
- **Phase 4B — Python Reference Adapters.** Complete 2026-05-03, `b5b6229` (tag `phase-4b-python-adapters`). Anthropic + OpenAI adapters; best-of-3 gate 3/3 ✓ both providers. See [`reports/phase-4b-completion-report.md`](./reports/phase-4b-completion-report.md).
- **Phase 5A — Tessera Core Engine (Streams A+B+C+D).** Merged `6c9950d` (tag `phase-5a-tessera-core`). `mc-recipe`, `mc-drivers` (6 source drivers), `mc-tessera` orchestrator, `WriteBatch` API. 502/0 tests. See [`handoffs/phase-5a-stream-d-handoff.md`](./handoffs/phase-5a-stream-d-handoff.md).
- **Phase 5B — LLM-Assisted Recipe Authoring.** Shipped (tag `phase-5b-llm-recipe-authoring`). 3 new plugin import skills, `mosaic-importer` agent, `/mosaic-import` command, `--mode propose-recipe` on both adapters; acceptance gate Anthropic 3/3 ✓ + OpenAI 3/3 ✓. See [`reports/phase-5b-completion-report.md`](./reports/phase-5b-completion-report.md).
- **Phase 3E–3G — Formula Language Expansion.** Shipped `334e0aa` (tag `phase-3e-3f-3g-formula-expansion`). Conditionals (`if/elif/else`), time-series ops (`lag/lead/cumsum/period_delta`), reference-data blocks (`lookup_table/segment_map`). ADRs 0011–0013 Accepted.
- **Phase 3F.1 — Runtime Time Anchor.** Shipped `8adbe2b`. `time_anchor` runtime parameter + time metadata validation in `mc-model`.
- **Phase 3H — Fitted-Model Evaluation.** Shipped `da644c2` (tag `phase-3h-fitted-model-evaluation`). `predict()`, `calibrate()`, `exp()`, `norm_cdf()` formula functions. 632/0 tests. See [`handoffs/phase-3h-handoff.md`](./handoffs/phase-3h-handoff.md).
- **Phase 5C — Driver Expansion + Cron + Incremental.** Shipped `e6f72ea` (tag `phase-5c-driver-expansion`). 5 additional Tessera source drivers, cron scheduling, incremental load support, ADR-0014 `time_format` enforcement in recipes. See [`handoffs/phase-5c-handoff.md`](./handoffs/phase-5c-handoff.md).
...
| Tests | `cargo test --workspace` | ✓ **632 / 0** (Phase 3H + 5C state at `99477ef`) |
| Determinism (10×) | `for i in $(seq 1 10); do cargo test --workspace -q ...; done` | ✓ 10 / 10 identical at 632 / 0 each run |
...
│   ├── mc-model/          model definition layer — YAML → mc_core::Cube (Phase 3A+)
│   ├── mc-cli/            CLI runner — `mc demo / model / tessera / mcp`
│   ├── mc-recipe/         recipe schema + validator + MC5xxx codes (Phase 5A)
│   ├── mc-drivers/        SourceDriver trait + source drivers (Phase 5A + 5C)
│   └── mc-tessera/        orchestrator — apply / dry-run / history / rollback (Phase 5A)
├── mosaic-plugin/         Claude Code plugin (skills, agents, commands, .mcp.json)
├── docs/                  this folder
