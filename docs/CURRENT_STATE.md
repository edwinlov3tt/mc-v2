...
**Last updated:** 2026-05-03 (Phase 3B implementation complete pending review. Test count 252 → 293; the headline gate `mc model lint crates/mc-model/examples/acme.yaml` exits 0 with zero warnings; all 15 ADR-0005 Decision 8 acceptance items closed; `mc-core` and `mc-fixtures` byte-for-byte unchanged from `phase-3a-model-definition-layer`. Awaiting user review + commit + tag — Phase 3B does NOT commit or tag itself per handoff hard rule.)
...
- **Phase 3B — Model QA, Linter, and Diagnostics.** **Implementation complete pending review** — see "What's shipping" above and [`reports/phase-3b-completion-report.md`](./reports/phase-3b-completion-report.md). Will move out of "queued" once committed + tagged.
- **Phase 3C — Friendly formula syntax (TBD).** Not started. `Revenue = Customers * AOV` strings compile down to `ParsedRuleBody`'s structured tree per ADR-0004 Decision 4. Phase 3C runs after Phase 3B per ADR-0005's "What this unlocks" rationale (diagnostics first, formulas second). No handoff yet.
