...
**Last updated:** 2026-05-03 (Phase 3C implemented; awaiting user review before commit/tag. Phase 3C flipped from `proposed` → `complete (uncommitted)`. Test count grew 293 → 328; Acme-name special case removed from `mc-cli`; equivalence test passes byte-identically across 2,520 canonical input coords + 9 goldens; `mc model test acme.yaml` runs in 32 ms wall-clock. Phase 3D (friendly formula syntax) flipped from `planned` → `proposed`. Earlier today: Phase 3B shipped at `f4f7fa8`, tagged `phase-3b-lint-and-diagnostics`; ADR-0006 Accepted with 13 project-owner acceptance amendments (9 from GPT + 4 from Claude Desktop); ADR-0004 Phase 3C label renamed to Phase 3D per ADR-0006 roadmap impact.)
...
- **Phase 3D — Friendly formula syntax — proposed.** `Revenue = Customers * AOV` strings compile down to `ParsedRuleBody`'s structured tree per ADR-0004 Decision 4. Originally named "Phase 3C" in ADR-0004; renamed to Phase 3D per ADR-0006 roadmap impact (the swap puts model-test fixtures ahead of formula-string ergonomics because the fixture work unblocks Phase 4 / 5 / 6 directly). No handoff yet.
...
| Tests | `cargo test --workspace` | ✓ **328 / 0** (was 293; +35 from Phase 3C: 9 csv unit + 2 equivalence_acme + 16 fixture_validators + 3 path_escape + 1 perf_gate + 4 schema_stability) |
| Determinism (10×) | `for i in $(seq 1 10); do cargo test --workspace -q ...; done` | ✓ 10 / 10 identical at 328 / 0 each run |
...
| Locked surfaces (mc-core) | `git diff phase-3b-lint-and-diagnostics -- crates/mc-core/` | ✓ 0 lines |
| Locked surfaces (mc-fixtures src) | `git diff phase-3b-lint-and-diagnostics -- crates/mc-fixtures/src/ crates/mc-fixtures/Cargo.toml` | ✓ 0 lines |
