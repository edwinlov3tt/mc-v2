...
> 7. All 9 diagnostic codes (MC2050-MC2056, MC3017-MC3018) fire on appropriate fixtures.
...
> 4. `exp()` function — the logistic sigmoid and Platt scaling both need `f64::exp()`. Should `exp(x)` be exposed as a standalone formula function too? (Recommend: yes, add it — it's one line in the evaluator and useful for decay models, growth formulas, etc.)
>
...
> New AST nodes: Predict, Calibrate (+ Exp if added per trigger #4)
...
- [ ] No new dependencies
- [ ] No unsafe
- [ ] `exp()` standalone function added (if trigger #4 approved — recommend yes)
